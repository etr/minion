...
**Action Items:**
- [ ] In SKILL.md, add `which pi` / `command -v pi` check via Bash tool as first step
- [ ] If Pi missing: present install offer with link to shittycodingagent.ai
- [ ] If user accepts install: run Pi installation command
- [ ] If user declines install: abort with clear message that Pi is required
- [ ] If Pi is present: proceed silently to invocation logic
