...
## Run: 2026-04-06 | FEATURE-claude-skills: claude-skills loading + pi-skills rename + inline overrides (3 iterations)

1. [ ] `major` **code-quality** | `test/test-claude-skills.sh` | test-coverage: No test exercises multiple `claude-skills` entries in a single run — every test uses exactly one entry, leaving the multi-skill composition path in `build_skills_section` untested -- Add a test with two skills (e.g., `skill-a` and `skill-b`) and assert both `### skill-a` and `### skill-b` headers appear in order with one `---` before the body
2. [ ] `major` **code-quality** | `test/*.sh` | duplication: `run_and_check`, `check`, `check_stdout_absent`, and mock-pi setup blocks are copied across test-claude-skills.sh, test-minion-run.sh, test-frontmatter-parsing.sh, and test-prompt-composition.sh -- Extract a shared `test/lib/test-helpers.sh` and source from each test file
3. [ ] `major` **code-simplifier** | `lib/minion-run.sh:195` | scalar-override-pattern: Six scalar-override merges spawn subshells with `[ ... ] && echo || echo` ternary; verbose, latent empty-string bug, hard to extend -- Extract a `pick_value` helper or use simple if-assignments
4. [ ] `major` **code-simplifier** | `lib/minion-run.sh:431` | prompt-composition-nesting: The has-skills/no-skills branches and body/extra-input nesting is hard to follow -- Extract a `compose_prompt` function or flatten with parameter expansion
5. [ ] `major` **code-simplifier** | `lib/minion-run.sh:388` | mixed-concerns: `build_skills_section` mixes resolution-and-error-reporting with content-assembly; 25-line error reporting block obscures happy path -- Extract `skill_resolution_error` helper
6. [ ] `major` **housekeeper** | `README.md:256` | doc-stale: Examples section omits the new `code-reviewer-with-skills.md` example file -- Add a bullet for it in the Examples section
7. [ ] `major` **performance** | `lib/minion-run.sh:416` | redundant-io: Each skill file is read twice (strip_frontmatter awk capture, then transform_skill_body sed pipe) — two subshells per skill -- Combine into one pipeline: `body_transformed=$(awk '...' "$resolved" | transform_skill_body)`
8. [ ] `minor` **code-quality** | `lib/minion-run.sh:323` | grammar: `AskUserQuestion` → `Ask the user` produces grammatically broken text in context like "Ask via AskUserQuestion when uncertain." → "Ask via Ask the user when uncertain." -- Use `ask_user` or rewrite as a phrase, also update the test that pins the broken output
9. [ ] `minor` **code-quality** | `lib/minion-run.sh:384` | misleading-set: `set +e` / `set -e` pair inside `build_skills_section` is misleading because outer script uses `set -uo pipefail` (no errexit) -- Remove the pair or add a comment
10. [ ] `minor` **code-quality** | `test/test-claude-skills.sh:611` | missing-test: No test for file-supplied `claude-skills` combined with `--prompt` override (EFFECTIVE_BODY resolution path goes untested) -- Add a test asserting prompt override replaces body when claude-skills present
11. [ ] `minor` **code-quality** | `README.md:107` | missing-doc: README documents `pi-skills` but does not mention `skills:` is accepted as a silent backwards-compat alias — existing users will not find confirmation their files still work -- Add a brief note (without recommending the legacy form)
12. [ ] `minor` **code-quality** | `test/test-prompt-composition.sh:160` | phase-numbering: Phase numbering skips Phase 4 (jumps from 3 to 5) -- Renumber sequentially or restore the missing phase
13. [ ] `minor` **code-quality** | `lib/minion-run.sh:489` | redundant-exit: Trailing `exit $?` is redundant; the last command's exit code becomes the script's exit code automatically -- Remove the trailing `exit $?`
14. [ ] `minor` **code-quality** | `lib/minion-run.sh:283` | per-call-realpath: `realpath -m "./.claude/skills"` is computed inside resolve_claude_skill on every call rather than once at startup -- Cache outside the function or document the asymmetry with $HOME-based roots
15. [ ] `minor` **code-quality** | `lib/minion-run.sh:281` | fallback-chain-ambiguity: `realpath -m || readlink -f` fallback chain — both fail on systems without either tool, but the comment implies a stricter guarantee than the chain provides -- Drop `readlink` fallback or document it as macOS/BSD portability
16. [ ] `minor` **test-quality** | `test/test-claude-skills.sh:388` | missing-test-variant: Path-traversal test covers bare `../escape` and `good-plugin:../escape` (bad skill-part) but not `../evil:skill` (bad plugin-part) -- Add a one-line test for the third variant
17. [ ] `minor` **test-quality** | `test/test-claude-skills.sh:453` | logic-in-test: `check_transformations` iterates over 25-element pattern arrays — pragmatic for asserting many sed substitutions in one run, but logic-in-test per the four-pillar framework -- Add a comment explaining the intentional choice
18. [ ] `minor` **test-quality** | `test/test-claude-skills.sh:475` | implementation-coupling: Test asserts the literal awkward string `"Ask via Ask the user when uncertain."` — paired with finding #8, this would break if the transform is improved -- Relax the assertion or fix the transform
19. [ ] `minor` **test-quality** | `test/test-claude-skills.sh:59` | parallelization-risk: `_stderr` filename is fixed; would corrupt under parallel test execution -- Use mktemp or counter suffix (consistent with existing test-frontmatter-parsing.sh and test-minion-run.sh patterns)
20. [ ] `minor` **test-quality** | `test/test-claude-skills.sh:923` | redundant-test: Phase 9a (`check_abs_allowed`) duplicates Phase 2e (`check_absolute_path`) — both accept absolute path inside `$HOME/.claude/skills/` -- Remove 9a and reference 2e, or add a comment justifying it as a paired positive control
21. [ ] `minor` **security** | `lib/minion-run.sh:292` | symlink-glob-traversal: `ls -d` glob for plugin cache (lines 292 and 314) follows symlinks and emits the resolved path without verifying it stays under `$HOME/.claude/plugins/cache/` -- Canonicalize via realpath after the glob and check prefix
22. [ ] `minor` **security** | `lib/minion-run.sh:265` | echo-flag-ambiguity: `valid_skill_name` uses `echo "$1" | grep -qE` — for inputs like `-e` or `-n`, echo may interpret as a flag -- Use `printf '%s'` or a pure-bash `case` pattern match
23. [ ] `minor` **security** | `lib/minion-run.sh:314` | glob-asymmetry: Bare-name lookup uses 3 wildcard segments; `plugin:skill` lookup uses 2 — search surface differs by skill format -- Document the layout in a comment, add a regression test
24. [ ] `minor` **security** | `lib/minion-run.sh` | no-frontmatter-warning: A skill file without `---` delimiters is silently treated as empty body — operator misconfig is hard to detect -- Emit a stderr warning and continue
25. [ ] `minor` **spec-alignment** | `lib/minion-run.sh:292` | undocumented-layout: Plugin cache directory layout assumption is implicit; no spec or CLAUDE.md note -- Document in CLAUDE.md
26. [ ] `minor` **architecture** | `lib/minion-run.sh:395` | dr-002-mild-violation: `build_skills_section` emits formatted "Searched the following locations" stderr — more verbose than the bash layer's "missing: x" / "file not found:" convention; UX should live in the skill -- Keep stderr terse and machine-parseable, let SKILL.md format the user message
27. [ ] `minor` **code-simplifier** | `lib/minion-run.sh:22` | flag-naming: 12 `*_SET` flag variables mix 0/1 integer semantics with later `"$X" = "1"` string comparison; inconsistent with NO_TOOLS_FLAG/NO_SESSION_FLAG/STREAM_FLAG conventions -- Pick one convention throughout (empty/non-empty, true/false, or 0/1)
28. [ ] `minor` **code-simplifier** | `lib/minion-run.sh:349` | duplicate-awk: `strip_frontmatter` and the FILE_BODY extraction (lines 171-172) use mirror-image awk programs reading the file independently -- Consolidate into a `split_frontmatter` helper or cross-reference with a comment
29. [ ] `minor` **code-simplifier** | `test/test-claude-skills.sh:225` | sequential-naming: `EMPTY_DIR`, `EMPTY_DIR2`...`EMPTY_DIR8` give no hint about purpose in failure output -- Rename to descriptive names like `EMPTY_DIR_MISSING_SKILL`
30. [ ] `minor` **code-simplifier** | `lib/minion-run.sh:426` | dead-code: `SKILLS_RC=$?` / `[ "$SKILLS_RC" != "0" ]` guard is unreachable because `build_skills_section` calls `exit 1` internally on error -- Remove the dead guard
31. [ ] `minor` **code-simplifier** | `lib/minion-run.sh:292` | double-negative: `allowed` flag (1=allowed) is tested with `[ "$allowed" -eq 0 ]` after the loop, reading as a double-negative -- Rename or restructure as `is_within_allowed_root` helper
32. [ ] `minor` **code-simplifier** | `test/test-claude-skills.sh:998` | missing-positive-assert: Phase 10 tests assert content does not appear in stdout but do not also assert exit 0 — would pass vacuously if minion-run.sh exited 1 -- Add an exit-code assertion
33. [ ] `minor` **code-simplifier** | `lib/minion-run.sh:300` | canonical-vs-entry: After absolute-path confinement validates `$canonical`, the function echoes `$entry` (raw, possibly symlink) rather than `$canonical` -- Return `$canonical` to keep what was validated
34. [ ] `minor` **code-simplifier** | `test/test-auto-dispatch.sh:995` | duplicate-cleanup: `_SENTINEL_ARGS_FILE` is removed both inside `_sentinel_args_test` (`rm -f`) and via the global `_CLEANUP_DIRS` trap -- Remove the inline rm and rely on the trap
35. [ ] `minor` **test-quality** | `test/test-auto-dispatch.sh:980` | redundant-assert: The sentinel regression test asserts both routing success and `--` presence; the routing assertion adds no regression value beyond what other tests already cover -- Drop the routing check and keep only the argv inspection
36. [ ] `minor` **housekeeper** | `CLAUDE.md:28` | doc-stale: Plugin structure tree omits `code-reviewer-with-skills.md` from examples/ and lists only 4 of 11 files in test/ -- Update the tree (or use a wildcard for tests)
37. [ ] `minor` **housekeeper** | `CLAUDE.md:47` | doc-stale: Component Formats describes `minion-run.sh` as "parses minion file YAML frontmatter, maps to Pi CLI flags, invokes Pi" — does not mention claude-skills loading or inline override flags -- Update to reflect expanded role
38. [ ] `minor` **performance** | `lib/minion-run.sh:292` | subshell-overhead: `ls -d ... | sort | head -1` spawns three processes per qualified skill resolution -- Use bash native glob expansion: `results=( $HOME/.claude/plugins/cache/*/$plugin_part/*/skills/$skill_part/SKILL.md ); echo "${results[0]}"`
39. [ ] `minor` **performance** | `lib/minion-run.sh:133` | dead-validation: `parse_field` runs `echo | grep -qE` for input validation 9 times during frontmatter parsing — all callers pass literal constants -- Replace with a `case` pattern match or remove entirely
40. [ ] `minor` **performance** | `lib/minion-run.sh:171` | two-pass-frontmatter: Frontmatter and body are extracted by separate awk passes -- Combine into a single awk call
